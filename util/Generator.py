...
class Generator:
    ...
    def addCompareStrings(self):
        self.code.append("void compareStrings() {")
        t1 = self.newTemp()
        t2 = self.newTemp()
        t3 = self.newTemp()
        t4 = self.newTemp()
        ...
        self.addGetStack(t4, t3)
        self.addAssignation(t5, "1")
        self.addGetHeap(t6, t2)
        self.addGetHeap(t7, t4)
        label1 = self.newLabel()
        label2 = self.newLabel()
        label3 = self.newLabel()
        label4 = self.newLabel()
        label5 = self.newLabel()
        label6 = self.newLabel()
        label7 = self.newLabel()
        self.addLabel(label3)
        self.addIf(t6, "-1", "!=", label1)
        self.addGoto(label2)
        self.addLabel(label1)
        self.addIf(t6, t7, "!=", label4)
        self.addGoto(label5)
        self.addLabel(label4)
        self.addAssignation(t5, "0")
        self.addGoto(label2)
        self.addLabel(label5)
        self.addExpression(t2, t2, "1", "+")
        self.addExpression(t4, t4, "1", "+")
        self.addGetHeap(t6, t2)
        self.addGetHeap(t7, t4)
        self.addGoto(label3)
        self.addLabel(label2)
        self.addIf(t7, "-1", "!=", label6)
        self.addGoto(label7)
        self.addLabel(label6)
        self.addAssignation(t5, "0")
        self.addLabel(label7)
        self.addSetStack("P", t5)
        self.code.append("return;")
        self.code.append("}\n")
